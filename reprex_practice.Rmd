...
# In this case, todd and sally don't have their names capitalized, so we want to do that.

df_new = df %>% 
  mutate(across(where(is.character), str_to_title))

# note that str_to_title is within across(), but not within where().
...
```{r}
# A constant cannot be mapped using aes()! aes() is only for variables.
ggplot(data= starwars, aes(y = homeworld, x = height)) +
  geom_col(fill = "purple")
# so x= and y= need to be in aes() because they are variables, but fill = purple is outside of aes() because I want everything to be purple. aes(fill = ""purple) would try and map to a variable, which would require a variable input.
